...
const Sidebar = ({ isOpen, toggleSidebar }) => {
    ...
    const menuItems = [
        ...
        { label: "My Properties", icon: MdOutlineEditCalendar, path: "/dashboard/my-properties" },
        { label: "My Favorites", icon: FaHeart, path: "/dashboard/my-favorites" },
        { label: "Messages", icon: MdOutlineMessage, path: "/dashboard/messages" },
        { label: "Add Property", icon: FaRegSquarePlus, path: "/dashboard/add-property" },
    ...
    return (
        <div>
            <div
                className={`fixed inset-0 z-20 bg-black opacity-50 lg:hidden ${isOpen ? "" : "hidden"
                    }`}
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.label}
                                to={item.path}
                                className={`group px-6 py-2 mt-4 flex items-center gap-2 rounded-lg ${isActive
                            ...
                            >
                                <item.icon
                                    className={`text-lg transition-colors ${isActive ? "text-white" : "text-gray-500 group-hover:text-white"
                                        }`}
                                />
